...
def independent_test():
    ...
    scores.append([auc,prauc, mcc,threshold, acc,tpr, tnr, ppv, npv,f1,f2,f5,f10])
    index.append("Train")
    col_names = ["AUROC","PRAUC", "MCC", "Threshold(by train)", "Accuracy","TPR(recall)", "TNR(specificity)", "PPV(precision)", "NPV","Fscore(beta=1)", "Fscore(beta=2)", "Fscore(beta=5)","Fscore(beta=10)"]
    result = pd.DataFrame(scores)
    result.index = index
    result.columns = col_names
    result.to_csv("Idependent_test_scores.csv")
    ...
    features = pd.read_csv("gene.average.beta.by.intensity.test.csv",index_col=0).index.tolist()
    print(len(features))
    shap = shaps[1]
